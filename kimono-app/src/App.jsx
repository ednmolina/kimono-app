...
function MapView({shops,selectedDay}){
  const [mapCity,setMapCity]=useState("Fukuoka");
  const cityShops=shops.filter(s=>s.city===mapCity);
  const c=CITY_CENTERS[mapCity];
  const zoom=mapCity==="Nagasaki"?15:mapCity==="Kumamoto"?13:13;

  const markers=cityShops.map(s=>{
    const isOpen=s.openDays.includes(selectedDay)&&s.status==="active";
    return `${s.lat},${s.lng},${isOpen?"red":"gray"},${encodeURIComponent(s.name+" ("+s.openTime+"-"+s.closeTime+")")}`;
  });

  const osmUrl=`https://www.openstreetmap.org/export/embed.html?bbox=${c.lng-c.span},${c.lat-c.span},${c.lng+c.span},${c.lat+c.span}&layer=mapnik&marker=${c.lat},${c.lng}`;

  return(
    <div>
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {CITIES.map(ci=>(<button key={ci} onClick={()=>setMapCity(ci)} style={{flex:1,padding:"8px 0",borderRadius:6,border:`1.5px solid ${mapCity===ci?CITY_ACCENTS[ci]:"#D4C4A8"}`,fontFamily:"'Cormorant Garamond',serif",fontSize:14,fontWeight:mapCity===ci?700:400,cursor:"pointer",background:mapCity===ci?CITY_ACCENTS[ci]:"transparent",color:mapCity===ci?"#FFFDF7":"#6B5D4A",transition:"all 0.25s"}}>{ci} ({shops.filter(s=>s.city===ci).length})</button>))}
      </div>

      <div style={{borderRadius:10,overflow:"hidden",border:"1px solid #D4C4A8",marginBottom:12}}>
        <iframe key={mapCity} width="100%" height="320" frameBorder="0" scrolling="no" style={{display:"block",filter:"sepia(0.15) saturate(0.9)"}}
          src={osmUrl} />
      </div>

      <div style={{fontSize:10,fontFamily:"'Lora',serif",color:"#A89880",textAlign:"center",marginBottom:10,fontStyle:"italic"}}>
        Map centered on {mapCity}. Shop locations listed below.
      </div>

      {[...cityShops].sort((a,b)=>{
        const ao=a.openDays.includes(selectedDay)&&a.status==="active"?0:1;
        const bo=b.openDays.includes(selectedDay)&&b.status==="active"?0:1;
        return ao-bo;
      }).map(shop=>{
        const isOpen=shop.openDays.includes(selectedDay)&&shop.status==="active";
        return(
          <a key={shop.id} href={`https://www.openstreetmap.org/?mlat=${shop.lat}&mlon=${shop.lng}#map=17/${shop.lat}/${shop.lng}`} target="_blank" rel="noopener noreferrer"
            style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",borderRadius:6,background:"#FFFDF7",border:"1px solid #E8DFD0",marginBottom:4,textDecoration:"none",opacity:isOpen?1:0.5,transition:"all 0.2s"}}>
            <div style={{width:10,height:10,borderRadius:"50%",background:isOpen?CITY_ACCENTS[shop.city]:"#B5A894",flexShrink:0}}/>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:14,fontWeight:600,color:"#3B2F1E"}}>{shop.name} <span style={{fontFamily:"'Lora',serif",fontSize:10,color:"#A89880",fontWeight:400}}>{shop.nameEn}</span></div>
              <div style={{fontFamily:"'Lora',serif",fontSize:10,color:"#8B7D6B"}}>{shop.address}</div>
            </div>
            <div style={{textAlign:"right",flexShrink:0}}>
              <div style={{fontSize:10,padding:"1px 6px",borderRadius:8,background:isOpen?"#E8F0E0":"#EDE8E0",color:isOpen?"#3D5A1E":"#8B7D6B",fontWeight:600,fontFamily:"'Lora',serif",marginBottom:2}}>{isOpen?"OPEN":"CLOSED"}</div>
...
export default function App(){
  ...
  return(<div style={{minHeight:"100vh",background:"linear-gradient(165deg,#F7F0E4 0%,#EDE5D6 40%,#F2EBE0 100%)",fontFamily:"'Lora',Georgia,serif",padding:"0 0 40px"}}>
    ...
    <div style={{maxWidth:480,margin:"0 auto",padding:"0 16px"}}>
      ...
      {tab==="map"&&<MapView shops={SHOPS} selectedDay={selectedDay}/>}
